...
void run(int taskId)
{
    ...
    cout<<"genome length= "<<task0.nslab+1<<endl;
    double dz1=task0.slab/task0.nslab; // genome length
    double dz0=task0.slab/(i-2); // parrett length
    multilayer ml0(i,l0,dz0,dz1);
    ml0.alpha0=task0.alpha0*M_PI/180.;
    ml0.gamma=task0.gamma*1e-20; // in J/angstrom^2 
    ml0.sigmad=task0.sigmad*0.5; // use 1/2 sigmad, since sigmad appears only once as 0.5*sigmad
    ml0.kappa=kappa_water*1.e30; // to angstrom^3/J
    ml0.dth=task0.dth*M_PI/180;
    ...
    //cout<<"rhoBulk0="<<rhoBulk0<<" rhoBulk1="<<rhoBulk1<<endl;
    //exit(0);
    unsigned int j;
    ...
    /*
     * use /dev/urandom to seed random(). Fall back to gettimeofday()
     */
    char random_buf[257];
...
if(task0.progress>=1) {
    ...
    genome2.copy(genome);
    /*fix me, this is not correct to do surface tension fitting*/
    ml0.update_sip(&genome);
