...
betaBulk0=0.;
    ...
    http    h0(APISTRUCT);
    h0.multiPartContentType("x-mixed-replace",NULL,h.boundaryString());
string data0;
        {
                char *lenstr;
...
while(ii++<data_len) {
    ...
    for(int ii=0;ii<ml0.xi.size();ii++) out1<<ml0.xi.at(ii)<<' '<<ml0.yi.at(ii)<<' '<<ml0.dyi.at(ii)<<endl;
    out1.close();
    }
...
//output picture the browser
                        h0.multiPartBoundary(NULL);
                        h0.contentType("image","png",NULL);
                        h0.cr();
			system("./plotmc0 tmpout");
                        ifstream        imagefile("tmpout.png");
                        h0.output(APISTRUCT,NULL,imagefile.rdbuf());
    }
    return 0;
}
